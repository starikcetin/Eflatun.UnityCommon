...
public class ObjectTracker : Singleton<ObjectTracker>
{
    ...
    public void RegisterObject (GameObject toRegister, GameObject prefab)
    {
        ...
        if (renderer)
        {
            ...
            TrackedObjectData newData = new TrackedObjectData(toRegister, renderer, localConvexHull);
    ...
    /// </summary>
    public int ByBox_AllNonAlloc(GameObject[] outputArray, Vector2 startPoint, Vector2 endPoint, int layerMask = Mathf2.LayerMaskAll)
    {
        var allDatas = ObjectTrackerDatabase.AllDatas;
    ...
    /// </summary>
    public int ByCircle_AllNonAlloc(GameObject[] outputArray, Vector2 center, float radius, int layerMask = Mathf2.LayerMaskAll)
    {
        var allDatas = ObjectTrackerDatabase.AllDatas;
    ...
    private bool Test_ByCircle(TrackedObjectData item, Vector2 center, float sqrRadius, Vector2 aabbMin, Vector2 aabbMax, int layerMask, bool checkForLayers)
    {
        if (checkForLayers)
        {
            //--- Step 1: Run the LayerMask test. ---
    ...
    {
        bool checkForLayers = layerMask != Mathf2.LayerMaskAll;   //if layermask is 1, that means everything is included, so we don't need to check.
    ...
    private TrackedObjectData Pick_ByCircle(List<TrackedObjectData> toPickFrom, Vector2 center, float radius, int layerMask)
    {
        bool checkForLayers = layerMask != Mathf2.LayerMaskAll;   //if layermask is 1, that means everything is included, so we don't need to check.
